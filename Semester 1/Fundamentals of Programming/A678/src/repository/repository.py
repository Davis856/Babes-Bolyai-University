    ...
    def remove_book(self, book_pos):
        ...
        for i in range(len(self.__book_data)):
            if book_pos == self.__book_data[i].id:
                book_pos = i + 1
        if book_pos - 1 < len(self.__book_data) and book_pos == self.__book_data[book_pos - 1].id:
...
class ClientRepository:
    ...
    def replace_client(self, client_pos, client_name):
        ...
        if client_pos - 1 > len(self.__client_data) or client_pos + client_id_add != self.__client_data[client_pos - 1].id:
            raise RepositoryException("Position does not exist")
        else:
