...
class UI:
    ...
    def switch_graph_ui(self):
        print("You are on graph number: {}".format(self._current))
    ...
    def get_number_of_vertices_ui(self):
        print("The number of vertices is: {}".format(self._graphs[self._current].vertices))

    def get_number_of_edges_ui(self):
        print("The numbr of edges is : {}".format(self._graphs[self._current].edges))
    ...
    def modify_cost_ui(self):
        ...
        vertex_y = int(input("Enter y = "))
        cost = int(input("Enter the cost of the edge: "))
        modified = self._graphs[self._current].change_cost(vertex_x, vertex_y, cost)
    ...
    def check_if_edge_ui(self):
        vertex_x = int(input("Enter x = "))
        vertex_y = int(input("Enter y = "))
        edge = self._graphs[self._current].find_edge(vertex_x, vertex_y)
    ...
    def copy_current_graph_ui(self):
        graph_copy = self._graphs[self._current].make_copy()
        self._graphs.append(graph_copy)

    def bfs_ui(self):
        start = int(input("Enter the first vertex: "))
        end = int(input("Enter the final vertex: "))
        print(self._graphs[self._current].bfs(start, end))

    def minimum_distance_ui(self):
        x = int(input("Enter x = "))
        y = int(input("Enter y = "))
        k = int(input("Enter the maximum value = "))
        while k > self._graphs[self._current].edges:
            print("The maximum distance must be shorter than the total number of edges.")
            k = int(input("Enter the maxim value = "))
        print("The cheapest path is:", self._graphs[self._current].minimum_distance(x, y, k))

    def minTreeKruskal_ui(self):
        mintree=[]
        mincost = self._graphs[self._current].minTreeKruskal(mintree)
        print(f"Minimal cost is: {mincost}.")
        print(f"Edges in minimal spanning tree")
        for edge in mintree:
            print(edge)

    def ui_get_hamiltonian_cycle(self):
        vertices_number = self._graphs[self._current].vertices
        if vertices_number == 0:
            print("The graph is empty.")
        else:
            try:
                cycle, cost = self._graphs[self._current].get_hamiltonian_cycle()
    ...
    def print_menu():
        print("Menu:\n"
              ...
              "23. Find the lowest length path between two vertices.\n"
              "24. Lowest cost walk.\n"
              "25. Minimum tree Kruskal.\n"
    ...
    def start(self):
        ...
        command_dict = {
                        "1": self.create_random_graph_ui, "2": self.read_graph_from_file_ui,
                        ...
                        "23": self.bfs_ui, "24": self.minimum_distance_ui,
                        "25": self.minTreeKruskal_ui, "26": self.ui_get_hamiltonian_cycle
                        }
        while not done:
            try:
                self.print_menu()
