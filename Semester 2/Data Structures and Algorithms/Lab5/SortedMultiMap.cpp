...
SortedMultiMap::SortedMultiMap(Relation r) : rel{r}, capacity{1}, rootPos{-1}, treeLength{0}, firstEmptyPos{0}
{
    ...
    {
        nodes[i]->key = NULL_KVALUE; ///nodes that have null kvalue as key act as nextPos list
        lefts[i] = i + 1;
        rights[i] = -1;
    }
    lefts[capacity - 1] = -1;
...
void SortedMultiMap::removeNodeWithOneDescendant(int nodePos, int parentPos) //lefts and rights of deleted node do not change
{
    ...
    else //remove root
    {
        ...
        else if (rights[nodePos] == -1) //root has only left descendants
        {
            delete[] nodes[nodePos];
            nodes[nodePos]->length = 0;
...
TValue SortedMultiMap::mostFrequent() const
{
    ...
    TValue maxx = NULL;
    int maxnr = 0, maxnr2 = 0;
    SMMIterator it = this->iterator();
    while (it.valid())
    {
        TElem nr2 = it.getCurrent();
        SMMIterator it2 = this->iterator();
        maxnr2 = 0;
        while (it2.valid())
        {
            TElem nr = it2.getCurrent();
            if (nr.second == nr2.second)
                maxnr2++;
